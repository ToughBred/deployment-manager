...
// githubClientPlain fetches deployment metadata from GitHub Releases.
type githubClientPlain struct {
	owner string
	repo  string
	token string
	http  *http.Client
...
// NewGithubClientPlain constructs a githubClient for the given repository.
// token may be empty for public repositories (rate limits apply).
func NewGithubClientPlain(owner, repo, token string) GitProvider {
	return &githubClientPlain{
		owner: owner,
		repo:  repo,
		token: token,
		http: &http.Client{
...
func (c *githubClientPlain) FetchLatestForEnvironment(ctx context.Context, releasePrefix, environment string) (meta DeploymentMetadata, err error) {
...
func (c *githubClientPlain) listReleases(ctx context.Context) ([]release, error) {
...
func (c *githubClientPlain) downloadMetadata(ctx context.Context, url string) (meta DeploymentMetadata, err error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		return meta, err
	}
...
func (c *githubClientPlain) addHeaders(req *http.Request) {
